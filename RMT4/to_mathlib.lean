...
lemma isCompact_segment {𝕜 E : Type*}
    [Ring 𝕜] [PartialOrder 𝕜] [IsOrderedRing 𝕜] [TopologicalSpace 𝕜] [ContinuousSub 𝕜]
  ...
  simpa only [segment_eq_image] using isCompact_Icc.image (by fun_prop)

lemma DifferentiableAt.deriv_eq_deriv_pow_div_pow {n : ℕ} (hn : n ≠ 0) {f g : ℂ → ℂ}
    (hg : f =ᶠ[𝓝 z] (g · ^ n)) (g_diff : DifferentiableAt ℂ g z) (fz_nonzero : f z ≠ 0) :
    deriv g z = deriv f z / (n * g z ^ (n - 1)) := by
  have h1 : g z ≠ 0 := fun h => fz_nonzero (by simp [hg.self_of_nhds, h, hn])
  have h2 : n * (g z) ^ (n - 1) ≠ 0 := by simp [h1, hn]
  rw [hg.deriv.self_of_nhds, deriv_fun_pow g_diff, eq_div_iff h2]
  ring
...
theorem integral_derivWithin_smul_comp
  ...
  refine integral_comp_smul_deriv'' hg.continuousOn (fun t ht => ?_) (hg.continuousOn_derivWithin'' le_rfl) hf
